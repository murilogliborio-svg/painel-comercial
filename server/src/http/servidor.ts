 ...
 * tratamento de erro. Construída sobre `node:http`, sem framework.
 *
 * A escolha por não usar framework é deliberada e tem uma razão de segurança
 * concreta: cada dependência de runtime é uma superfície de supply chain que
 * alguém precisa auditar e manter atualizada. O que um framework entrega
 * aqui — roteamento por método e caminho, parsing de JSON, middleware — são
 * ~250 linhas legíveis. TLS, compressão, HTTP/2 e servir estático com cache
 * ficam a cargo do proxy reverso (Caddy), que é software especializado nisso.
 */
...
export interface Requisicao {
  ...
  requestId: string;
  /** Corpo JSON já validado como objeto, quando aplicável. */
  corpo: unknown;
  /** Corpo binário bruto, para upload. */
  binario: Buffer | null;
  bruto: IncomingMessage;
  /** Espaço para dados anexados pelos middlewares (usuário, sessão...). */
...
/** Erro com status HTTP. Qualquer outro erro vira 500 sem vazar detalhe. */
...
export class Roteador {
  ...
  resolver(metodo: string, caminho: string): { handler: Handler; params: Record<string, string> } | null {
    ...
    // Caminho existe mas método não: 405, não 404. Ajuda quem integra.
...
/**
 * A CSP não permite `unsafe-inline`. Consequência prática: nenhum `<script>`
 * ou `onclick=` inline no frontend — todo JS vem de arquivo próprio. É o que
 * transforma a CSP de enfeite em barreira real contra XSS.
 */
...
async function lerCorpo(req: IncomingMessage, maxBytes: number): Promise<Buffer> {
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on('data', (c: Buffer) => {
      total += c.length;
      // Checa a cada pedaço: um cliente pode mentir no Content-Length.
      if (total > maxBytes) {
        reject(erro.grande());
        req.destroy();
        return;
...
function ipDe(req: IncomingMessage, confiarProxy: boolean): string {
  if (confiarProxy) {
    ...
    // O primeiro da lista é o cliente; os demais são proxies.
...
export function criarServidor(opts: OpcoesServidor): Server {
  ...
  return createServer(async (bruto: IncomingMessage, res: ServerResponse) => {
    ...
    try {
      ...
      // HEAD é resolvido como GET e respondido sem corpo (RFC 9110 §9.3.2).
      // Ferramentas de monitoramento e proxies usam HEAD; responder 405 faria
      // um health check legítimo parecer falha.
      ...
      // Corpo: JSON por padrão, binário quando o upload declara octet-stream.
...
function respostaDeErro(e: unknown, requestId: string): Resposta {
  ...
  // Erro não previsto: o cliente recebe apenas o id para correlacionar com o log.
  return {
    status: 500,
    corpo: {
      erro: 'interno',
...
function enviar(
  ...
  // Content-Length continua sendo o tamanho que o GET teria, mesmo em HEAD.
